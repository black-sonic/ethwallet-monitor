...
def get_token_prices(symbols):
    ...
    selected_ids = [ids[s] for s in symbols if s in ids]
    if not selected_ids:
        return {}

    try:
        url = f"https://api.coingecko.com/api/v3/simple/price?ids={','.join(selected_ids)}&vs_currencies=usd"
        res = requests.get(url)
        return {s: res.json().get(ids[s], {}).get("usd", 0) for s in symbols if s in ids}
    except:
        return {}

def get_token_holdings(network_key, address):
    network = NETWORK_CONFIG[network_key]
    url = f"{network['url']}?module=account&action=tokentx&address={address}&apikey={network['apikey']}"
    ...
    try:
        ...
        tokens = defaultdict(lambda: {"balance": 0, "decimals": 0, "symbol": ""})
        for tx in data["result"]:
            ...
            tokens[symbol]["decimals"] = decimals
            tokens[symbol]["symbol"] = symbol

        result = {}
        for sym, info in tokens.items():
            result[sym] = round(info["balance"] / (10 ** info["decimals"]), 6)
        return result
    ...
    while True:
        ...
        try:
            ...
            if data.get("status") == "1":
                ...
                tokens = get_token_holdings(network_key, address)
                if tokens:
                    ...
                    for sym, bal in tokens.items():
                        ...
                        if p:
                            usd_val = bal * p
                            print(f"- {sym}: {bal:.6f} (~ ${usd_val:.2f} @ ${p:.2f})")
                        else:
                            print(f"- {sym}: {bal:.6f}")
                else:
